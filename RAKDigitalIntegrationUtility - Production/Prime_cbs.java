...
public class Prime_cbs implements Runnable
{
   ...
   // private File exceFile;
	...
	public String readExcelForPrime(String finalSourcePath,String newFilename,String cabinetName, String sJtsIp, String iJtsPort, String sessionId)
	{
		String status="";
	            ...
	            for (int row = 1; row < rows-1; row++)
	            {
	            	...
	            	*/
	    			...
	    			while(read.hasNextLine())
	    			{
	    				...
	    				if(str_Card_Type.contains("CREDIT")){
	    					Query_for_ecrn = "select ecrn as 'ECRN',WI_name as 'Wi_name'  from NG_DCC_EXTTABLE with(nolock) where ECRN = '"+elite_crn+"'";
	    				}
	    				else{
	    				...
	    				if(apInsertMaincode.equalsIgnoreCase("0")){
	    					...
	    					if(str_Card_Type.contains("CREDIT")){
	    						updateExternalTable("NG_DCC_EXTTABLE","Courier_Flag","'File_received'","WI_name='"+Wi_name+"'", sJtsIp, iJtsPort, cabinetName, sessionId);
	    					}else{
	    					...
	    					if(str_Card_Type.contains("CREDIT")){
	    						updateExternalTable("NG_DCC_EXTTABLE","Courier_Flag","'File_Error'","WI_name='"+Wi_name+"'", sJtsIp, iJtsPort, cabinetName, sessionId);
	    					}else{
...
// End	
	...
	public String ReadCSBEFile(String finalSourcePath,String newFilename,String cabinetName, String sJtsIp, String iJtsPort, String sessionId)
	{
				...
				while(read.hasNextLine())
				{
					...
					if(apInsertMaincode.equalsIgnoreCase("0"))
					{
						...
						if(ChequeBk_ref.equals(chqBk_ref))
							updateExternalTable("NG_DAO_EXTTABLE","Is_CBS","'Y'","WI_name='"+WI_name+"'", sJtsIp, iJtsPort, cabinetName, sessionId);
					...
					{
						DAO_prime.DAOPrimeLogger.debug("ApInsert failed: "+apInsertMaincode);
						updateExternalTable("NG_DAO_EXTTABLE","Is_CBS","'E'","WI_name='"+WI_name+"'", sJtsIp, iJtsPort, cabinetName, sessionId);
						status="fail";
						break;
					}
					//Ap insert call end:
	...
	private void updateExternalTable(String tablename, String columnname,String sMessage, String sWhere, String jtsIP, String jtsPort, String cabinetName,String sessionId){
			...
			if(files.length == 0){
				...
				for(int i=files.length-1;i>=0;i--){
					...
					if(msg.equalsIgnoreCase("")){
						...
						//start
						sendMail(cabinetName,sessionID,serverPort,jtsIP,jtsPort,file);
						continue;
					}
						...
						updateStatus = readExcelForPrime(finalSourcePath,newFilename,cabinetName,serverIP,serverPort,sessionID);
			...
			else
			{
				for(int i=files.length-1;i>=0;i--)	
				{
					...
					else
					{
						...
						String finalSourcePath = TempinProgressPath+"\\"+newFilename;
						updateStatus = ReadCSBEFile(finalSourcePath,newFilename,cabinetName,serverIP,serverPort,sessionID);
	...
	*/
	...
	public static  void sendMail(String cabinetName, String sessionId ,String wiName,String jtsIp,String jtsPort,String file)throws Exception
    {
        XMLParser objXMLParser = new XMLParser();
        String sInputXML="";
        String sOutputXML="";
		...
		while(sessionCheckInt<loopCount)
        {
            ...
            {	
            	...
            	String strValues="'"+fromMailID+"','"+toMailID+"','"+mailSubject+"','"+FinalMailStr+"','text/html;charset=UTF-8','1','N','CUSTOM','TRIGGER','"+CommonMethods.getdateCurrentDateInSQLFormat()+"','44','"+wiName+"','1','1','0'";
