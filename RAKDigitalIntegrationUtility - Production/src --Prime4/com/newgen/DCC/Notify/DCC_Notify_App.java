		...
		try
		{
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0)
			{
				for(int i=0; i<fetchWorkitemListCount; i++)
				{
				    ...
				    String DBQuery ="select  FIRCO_Flag,FircoUpdateAction,EFMS_Status,FTS_Ack_flg,Dectech_Decision,Decision,Final_Limit,Card_Limit,ADDITIONAL_DOCUMENT_REQUIRED,prospect_id,NOTIFY_DEH_IDENTIFIER,Card_Product_Sub_Type," +
				    		...
				    		"FinalTAI,DBR_lifeStyle_expenses,Output_TotalDeduction,Output_Stress_BufferAmt,cancel_in_cooling_period,Decline_reason,Product,Nationality,FATCA_Tin_Number,IBPS_Expiry from NG_DCC_EXTTABLE with(nolock) where WI_name='" + processInstanceID + "'";
