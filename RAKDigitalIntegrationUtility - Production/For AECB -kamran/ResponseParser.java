					                    	 ...
					                    	 if (flag1.equalsIgnoreCase("true"))
							                    ...
							                    //	subtag_single = "CBAccountNo,DPAccountNo,ProviderNo,AccountType,Phase,IBAN,StartDate,DateOfLastUpdate";
							                    	...
							                    	flag1 = commonParseProduct(parseXml, tagName, wi_name, returnType, sTableName, wrapperIP, wrapperPort, sessionId, cabinetName, subTagName, prod, subprod, cifId,
							                    	 ...
							                    	 if (flag1.equalsIgnoreCase("true"))
									                    {
									                    	tagName="SalaryCreditHistory"; 
									                    	subTagName = "";
									                    	sTableName = "ng_dcc_cust_extexpo_SalCreditHis";
									                    	DCCSystemIntegrationLog.DCCSystemIntegrationLogger.debug("SalaryCreditHistory");
									                    	flag1 = commonParseProduct(parseXml, tagName, wi_name, returnType, sTableName, wrapperIP, wrapperPort, sessionId, cabinetName, subTagName, prod, subprod, cifId,
									                                 cust_type, subtag_single);
                ...
                for (Map.Entry<String, String> entry : map.entrySet())
                  {
                    ...
                    else if (sTableName.equalsIgnoreCase("ng_dcc_cust_extexpo_SalCreditHis"))
                    {
                    	DCCSystemIntegrationLog.DCCSystemIntegrationLogger.debug("inside ng_dcc_cust_extexpo_SalCreditHis table condition " );
                    	String map_values[] = entry.getKey().split("~");
                    	String CBAccountNo = map_values[1];
                    	String DPAccountNo = map_values[2];
                    	columnName = valueArr[0]+",wi_name,CBAccountNo,DPAccountNo";
                    	columnValues = valueArr[1] + ",'"+wi_name+"','"+ CBAccountNo +"','" + DPAccountNo +"'";
                        sWhere = "Wi_Name='" + wi_name +"' AND ReferenceDate='"+ map_values[0] +"'AND CBAccountNo= '"+map_values[1]+"'";
    ...
    public static Map<String, String> getTagDataParent_deep_new(String parseXml, String tagName, String sub_tag, String subtag_single) {
		...
		try {
			...
			for (int i = 0; i < nList_loan.getLength(); i++) {
				...
				else if ("SalaryCreditHistory".equalsIgnoreCase(tagName)) {
					id = ch_nodeList.item(0).getTextContent();
				    NodeList nList_loan1 = doc.getElementsByTagName("SalaryCreditDetails");
					NodeList ch_nodeList1 = nList_loan1.item(0).getChildNodes();
					id = id+"~"+ch_nodeList1.item(3).getTextContent()+"~"+ch_nodeList1.item(4).getTextContent();
				 } 
